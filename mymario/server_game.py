...
class Game:
    def __init__(self, nop, game_map="test_map"):
        ...
        self.map.map_array[3][10:16] = "Server"
        # self.map.view_map()
        # exit(1)
        self.controls = Controls()

        inp_lr_thread = DaemonThread(target=self.get_input_for_player, args=(self.player_id,))
        # inp_up_thread = DaemonThread(target=self.get_up_input_for_player, args=(self.player_id,))
        time_thread = DaemonThread(target=self.decrease_time, args=(0,))
    ...
    def get_up_input_for_player(self, player_id):
        while True:
            if keyboard.is_pressed(self.controls.player[player_id].UP):
                self.server.send({"object": "player", "player_id": self.player_id, "command": "up"})
    ...
    def get_map_class(game_map):
        ...
        try:
            ...
            print(
                "Sorry given map " + game_map + " not found, Strictly follow naming comventions for the map modules.")
            exit(1)
        return map_class
